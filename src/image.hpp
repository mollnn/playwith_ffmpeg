...
struct Image_RGB888
{
    ...
    std::vector<std::vector<vec3_uint8>> buffer;
    ...
    Image_RGB888(int sx = 0, int sy = 0) : size_x(sx), size_y(sy)
    {
        this->reallocate();
    }

    void Set(int x, int y, vec3_uint8 color)
    {
        assert(0 <= x && x < size_x);
        assert(0 <= y && y < size_y);
        buffer[x][y] = color;
    ...
    vec3_uint8 Add(int x, int y, vec3_uint8 color)
    {
        // std::cerr << "Add " << x << " " << y << " " << color.x << " " << color.y << " " << color.z << std::endl;
